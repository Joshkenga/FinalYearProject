...
async function sortProjects(filterType){
    const request  = new Request(`/dashboard/${filterType}`, {
        method:'POST',
        headers:{
            'X-CSRFToken':csrfToken.value
        },
        body:JSON.stringify({'projectsFilter':filterType})
    })
...
async function sortProjectsByFilterAndCategory(category, filterType){
...
relatedTags.forEach(tag =>{
    ...
    tag.addEventListener('click', ()=>{
        sortProjectsByFilterAndCategory(chosenCategory, activeFilter);
    })
})

// Call displayNotifications on page load or when needed
